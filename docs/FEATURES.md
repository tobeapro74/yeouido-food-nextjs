...
### 3.4 AI 리뷰 분석 섹션 (2026.05.18 추가)
- Google 리뷰 캐시 완료 후 자동 실행 (순서 보장)
- Claude Haiku로 리뷰 텍스트 분석
- **대표 메뉴**: 리뷰에 언급된 음식/메뉴명 최대 3개 (주황색 태그)
- **대표 리뷰**: 가장 유익한 리뷰 핵심 요약 최대 3개 (파란 카드)
- 분석 결과 MongoDB `ai_menu_summary` 컬렉션에 7일 캐싱
- 리뷰가 없거나 분석 실패 시 섹션 미표시

### 3.5 Google 리뷰 섹션
- Google Places API 연동
- 평점 및 리뷰 수 표시
- 리뷰 내용 미리보기
- 리뷰 로딩 완료 시 AI 분석 섹션에 신호 전달 (`onReviewsReady`)
